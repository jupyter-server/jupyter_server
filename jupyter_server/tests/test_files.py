...
class FilesTest(ServerTestBase):
    def test_hidden_files(self):
        ...
        for d in dirs:
            path = pjoin(root_dir, d.replace('/', os.sep))
    ...
    def test_contents_manager(self):
        ...
        with io.open(pjoin(root_dir, 'testnb.ipynb'), 'w', 
            encoding='utf-8') as f:
            write(nb, f, version=4)

        with io.open(pjoin(root_dir, 'test.bin'), 'wb') as f:
            f.write(b'\xff' + os.urandom(5))
            f.close()

        with io.open(pjoin(root_dir, 'test.txt'), 'w') as f:
            f.write(u'foobar')
            f.close()
    ...
    def test_download(self):
        ...
        text = 'hello'
        with open(pjoin(root_dir, 'test.txt'), 'w') as f:
            f.write(text)
    ...
    def test_old_files_redirect(self):
        ...
        os.mkdir(pjoin(root_dir, 'files'))
        os.makedirs(pjoin(root_dir, 'sub', 'files'))
        
        for prefix in ('', 'sub'):
            with open(pjoin(root_dir, prefix, 'files', 'f1.txt'), 'w') as f:
                f.write(prefix + '/files/f1')
            with open(pjoin(root_dir, prefix, 'files', 'f2.txt'), 'w') as f:
                f.write(prefix + '/files/f2')
            with open(pjoin(root_dir, prefix, 'f2.txt'), 'w') as f:
                f.write(prefix + '/f2')
            with open(pjoin(root_dir, prefix, 'f3.txt'), 'w') as f:
                f.write(prefix + '/f3')
