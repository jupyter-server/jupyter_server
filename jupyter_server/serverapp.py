...
msg = "The Jupyter Server requires tornado >= 4.0"
try:
    import tornado
except ImportError:
...
class JupyterWebApplication(web.Application):
...
class JupyterServerListApp(JupyterApp):
    version = __version__
    description="List currently running jupyter servers."
    ...
    def start(self):
        ...
        for serverinfo in list_running_servers(self.runtime_dir):
            if self.json:
                ...
                print(serverinfo['url'], "::", serverinfo['root_dir'])
...
# Add file content manager flags
...
class ServerApp(JupyterApp):
    ...
    description = """
        ...
        This launches a Tornado based HTML Jupyter Server.
    """
    examples = _examples
    aliases = aliases
    flags = flags
    appclass = JupyterWebApplication

    classes = [
        KernelManager, Session, MappingKernelManager,
        ContentsManager, FileContentsManager, KernelSpecManager,
    ]
    flags = Dict(flags)
    ...
    allow_root = Bool(False, config=True,
        help="Whether to allow the user to run the server as root."
    )
    ...
    port = Integer(8888, config=True,
        help="The port the Jupyter server will listen on."
    )
    ...
    cookie_secret = Bytes(b'', config=True,
        ...
        By default this is a new random number every time you start the application.
    ...
    contents_manager_class = Type(
        ...
        config=True,
        help='The content manager class to use.'
    )
    ...
    root_dir = Unicode(config=True,
        help="The root directory for content and kernels."
    )
    ...
    def _root_dir_validate(self, proposal):
        ...
        if not os.path.isdir(value):
            raise TraitError("No such root dir: %r" % value)
        return value

    @observe('root_dir')
    def _update_root_dir(self, change):
        """Do a bit of validation of the root directoy."""
        new = change['new']
        self.config.FileContentsManager.root_dir = new
        self.config.MappingKernelManager.root_dir = new

    nbserver_extensions = Dict({}, config=True,
        help=("Dict of Python modules to load as Jupyter server extensions."
    ...
    def init_webapp(self):
        ...
        for port in random_ports(self.port, self.port_retries+1):
            try:
                ...
                break
        if not success:
            self.log.critical('ERROR: the Jupyter server could not be started because '
    ...
    def _confirm_exit(self):
        ...
        sys.stdout.write("Shutdown this jupyter server (y/[n])? ")
        sys.stdout.flush()
    ...
    def notebook_info(self):
        ...
        return info + "The Jupyter server is running at: %s" % self.display_url
