...
from ...files.handlers import ExtractDirectoryHandler
...
class ContentsManager(LoggingConfigurable):
    ...
    hide_globs = List(
        Unicode(),
        [u"__pycache__", "*.pyc", "*.pyo", ".DS_Store", "*.so", "*.dylib", "*~",],
        config=True,
        help="""
    ...
    def _default_checkpoints_kwargs(self):
        return dict(parent=self, log=self.log,)
    ...
    directory_handler_params = Dict(
        config=True,
        help="""Extra parameters to pass to directory_handler_class.
        """,
    )

    extract_directory_handler_class = Type(
        ExtractDirectoryHandler,
        klass=RequestHandler,
        allow_none=True,
        config=True,
        help="""handler class to use when serving raw directory requests.
    ...
    extract_directory_handler_params = Dict(
        config=True,
        help="""Extra parameters to pass to extract_directory_handler_class.
        """,
    )
    ...
    def get_extra_handlers(self):
        ...
        if self.files_handler_class:
            ...
            handlers.append(
                (
                    r"/directories/(.*)",
                    self.directory_handler_class,
                    self.directory_handler_params,
                )
            )
            handlers.append(
                (
                    r"/extract-directories/(.*)",
                    self.extract_directory_handler_class,
                    self.extract_directory_handler_params,
                )
            )
        return handlers
