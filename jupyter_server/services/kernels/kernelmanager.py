...
class KernelInterface(LoggingConfigurable):
    ...
    async def _handle_kernel_restarted(self, data):
    ...
    async def restart(self):
        ...
        # new client.
        await self.restarter.do_restart()
        # Resume monitoring the kernel for auto-restart
        self.restarter.start()
        await self._client_connected_evt.wait()
...
class MappingKernelManager(LoggingConfigurable):
    ...
    def get_kernel(self, kernel_id):
        """Return KernelInterface instance corresponding to kernel_id.
           If kernel_id is not currently managed, HTTPError 404 is raised.
        """
    ...
    async def start_kernel(self, kernel_id=None, path=None, kernel_name=None, **kwargs):
        ...
        if kernel_id is None:
            kernel_id = await ensure_async(self.start_launching_kernel(path=path, kernel_name=kernel_name, **kwargs))
            await self.get_kernel(kernel_id).client_ready()
        else:
            self._check_kernel_id(kernel_id)
    ...
    async def start_launching_kernel(self, path=None, kernel_name=None, **kwargs):
        ...
        kernel = await KernelInterface.launch(kernel_name, self.kernel_finder, **kwargs)
    ...
    def _check_kernel_id(self, kernel_id):
        """Check that kernel_id exists and raise 404 if not."""
    ...
    async def cull_kernel_if_idle(self, kernel_id):
        ...
        if kernel.last_activity is not None:
            ...
            connections = kernel.n_connections
            is_idle_connected = self.cull_connected or connections == 0
            ...
            if (is_idle_time and is_idle_execute and is_idle_connected):
                ...
                await self.shutdown_kernel(kernel_id, now=True)
