...
class ExtensionApp(JupyterApp):
                ...
                if c in value:
                    raise ValueError(
                        ...
                        "{invalid_chars}.".format(
                            name=value,
                            invalid_chars=ExtensionApp.INVALID_EXTENSION_NAME_CHARS,
                        )
                    )
            return value
        raise ValueError(
            "Extension name must be a string, found {type}.".format(type=type(value))
        )
    ...
    settings = Dict(help=_("""Settings that will passed to the server.""")).tag(
        config=True
    )

    handlers = List(help=_("""Handlers appended to the server.""")).tag(config=True)
    ...
    open_browser = Bool(True, help=_("Should the extension open a browser window?"))
    ...
    def launch_instance(cls, argv=None, **kwargs):
        ...
        if subapp:
            ...
            extension = cls.load_jupyter_server_extension(
                serverapp, argv=args, **kwargs
            )
            # Start the ioloop.
            extension.start()
