...
async def test_create_retrieve_config(fetch):
    ...
    response2 = await fetch("api", "config", "example", method="GET",)
...
async def test_modify(fetch):
    ...
    await fetch("api", "config", "example", method="PUT", body=json.dumps(sample))
...
async def test_get_unknown(fetch):
    response = await fetch("api", "config", "nonexistant", method="GET",)
