...
async def test_get_kernelspecs(fetch, kernelspecs):
    r = await fetch("api", "kernelspecs", "Sample", method="GET")
    model = json.loads(r.body.decode())
...
async def test_get_kernelspec_spaces(fetch, kernelspecs):
    r = await fetch("api", "kernelspecs", "sample%202", method="GET")
    model = json.loads(r.body.decode())
...
async def test_get_nonexistant_kernelspec(fetch, kernelspecs):
    with pytest.raises(tornado.httpclient.HTTPClientError) as e:
        await fetch("api", "kernelspecs", "nonexistant", method="GET")
    assert expected_http_error(e, 404)


async def test_get_kernel_resource_file(fetch, kernelspecs):
    r = await fetch("kernelspecs", "sAmple", "resource.txt", method="GET")
...
async def test_get_nonexistant_resource(fetch, kernelspecs):
    with pytest.raises(tornado.httpclient.HTTPClientError) as e:
        await fetch("kernelspecs", "nonexistant", "resource.txt", method="GET")
    assert expected_http_error(e, 404)

    with pytest.raises(tornado.httpclient.HTTPClientError) as e:
        await fetch("kernelspecs", "sample", "nonexistant.txt", method="GET")
    assert expected_http_error(e, 404)
