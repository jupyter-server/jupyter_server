...
def ws_fetch(auth_header, http_port):
    ...
    def client_fetch(*parts, headers={}, params={}, **kwargs):
        ...
        urlparts = urlparts._replace(path=path, query=urllib.parse.urlencode(params))
        url = urlparts.geturl()
        # Add auth keys to header
        headers.update(auth_header)
        # Make request.
        req = tornado.httpclient.HTTPRequest(
            url, headers=auth_header, connect_timeout=120
        )
...
async def test_default_kernels(fetch):
    r = await fetch("api", "kernels", method="POST", allow_nonstandard_methods=True)
...
async def test_kernel_handler(fetch):
    ...
    r = await fetch("api", "kernels", kernel_id, method="GET")
    kernel = json.loads(r.body.decode())
    assert r.code == 200
    assert isinstance(kernel, dict)
    assert "id" in kernel
    ...
    with pytest.raises(tornado.httpclient.HTTPClientError) as e:
        r = await fetch("api", "kernels", bad_id, method="GET")
    ...
    bad_id = "111-111-111-111-111"
    with pytest.raises(tornado.httpclient.HTTPClientError) as e:
        r = await fetch("api", "kernels", bad_id, method="DELETE")
...
async def test_connection(fetch, ws_fetch, http_port, auth_header):
    ...
    # Get kernel info
    r = await fetch("api", "kernels", kid, method="GET")
