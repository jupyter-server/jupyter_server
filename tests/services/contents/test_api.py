...
async def test_list_notebooks(fetch, contents, path, name):
    response = await fetch("api", "contents", path, method="GET",)
...
async def test_get_dir_no_contents(fetch, contents, path, name):
    response = await fetch(
        "api", "contents", path, method="GET", params=dict(content="0",)
    )
...
async def test_list_nonexistant_dir(fetch, contents):
    with pytest.raises(tornado.httpclient.HTTPClientError):
        await fetch(
            "api", "contents", "nonexistant", method="GET",
        )
...
async def test_get_nb_contents(fetch, contents, path, name):
    ...
    r = await fetch("api", "contents", nbpath, method="GET", params=dict(content="1"))
...
async def test_get_nb_no_contents(fetch, contents, path, name):
    nbname = name + ".ipynb"
    nbpath = (path + "/" + nbname).lstrip("/")
    r = await fetch("api", "contents", nbpath, method="GET", params=dict(content="0"))
    model = json.loads(r.body.decode())
    assert model["name"] == nbname
    assert model["path"] == nbpath
    assert model["type"] == "notebook"
...
async def test_get_nb_invalid(contents_dir, fetch, contents):
    ...
    r = await fetch("api", "contents", nbpath, method="GET",)
    model = json.loads(r.body.decode())
    assert model["path"] == nbpath
    assert model["type"] == "notebook"
    assert "content" in model
...
async def test_get_contents_no_such_file(fetch):
    with pytest.raises(tornado.httpclient.HTTPClientError) as e:
        await fetch(
            "api", "contents", "foo/q.ipynb", method="GET",
        )
...
async def test_get_text_file_contents(fetch, contents, path, name):
    ...
    r = await fetch("api", "contents", txtpath, method="GET", params=dict(content="1"))
    model = json.loads(r.body.decode())
    ...
    with pytest.raises(tornado.httpclient.HTTPClientError) as e:
        await fetch(
            "api", "contents", "foo/q.txt", method="GET",
        )
...
async def test_get_binary_file_contents(fetch, contents, path, name):
    ...
    r = await fetch("api", "contents", blobpath, method="GET", params=dict(content="1"))
    model = json.loads(r.body.decode())
...
async def test_create_untitled_txt(fetch, contents, contents_dir):
    ...
    r = await fetch("api", "contents", path, name, method="GET")
    model = json.loads(r.body.decode())
    assert model["type"] == "file"
    assert model["format"] == "text"
...
async def test_upload_txt(fetch, contents, contents_dir):
    ...
    await fetch("api", "contents", path, name, method="PUT", body=json.dumps(model))
...
async def test_delete(fetch, contents, contents_dir, path, name):
    nbname = name + ".ipynb"
    nbpath = (path + "/" + nbname).lstrip("/")
    r = await fetch("api", "contents", nbpath, method="DELETE",)
    assert r.code == 204
...
async def test_delete_dirs(fetch, contents, folders):
    ...
    for name in sorted(folders + ["/"], key=len, reverse=True):
        r = await fetch("api", "contents", name, method="GET")
        # Get JSON blobs for each content.
        listing = json.loads(r.body.decode())["content"]
        # Delete all content
        for model in listing:
            await fetch("api", "contents", model["path"], method="DELETE")
    # Make sure all content has been deleted.
    r = await fetch("api", "contents", method="GET")
    model = json.loads(r.body.decode())
    assert model["content"] == []
...
async def test_delete_non_empty_dir(fetch, contents):
    # Delete a folder
    await fetch("api", "contents", "å b", method="DELETE")
    # Check that the folder was been deleted.
    with pytest.raises(tornado.httpclient.HTTPClientError) as e:
        await fetch("api", "contents", "å b", method="GET")
    assert expected_http_error(e, 404)
...
async def test_rename(fetch, contents, contents_dir):
    ...
    r = await fetch("api", "contents", path, method="GET")
...
async def test_checkpoints_follow_file(fetch, contents):
    ...
    # List checkpoints
    r = await fetch("api", "contents", path, name, "checkpoints", method="GET",)
...
async def test_save(fetch, contents):
    r = await fetch("api", "contents", "foo/a.ipynb", method="GET")
    model = json.loads(r.body.decode())
...
async def test_checkpoints(fetch, contents):
    path = "foo/a.ipynb"
    resp = await fetch("api", "contents", path, method="GET")
    ...
    # Save it.
    nbmodel = {"content": nb, "type": "notebook"}
    resp = await fetch("api", "contents", path, method="PUT", body=json.dumps(nbmodel))

    # List checkpoints
    r = await fetch("api", "contents", path, "checkpoints", method="GET")
    cps = json.loads(r.body.decode())
    assert cps == [cp1]
    ...
    # Delete cp1
    r = await fetch("api", "contents", path, "checkpoints", cp1["id"], method="DELETE")
    assert r.code == 204
...
async def test_file_checkpoints(fetch, contents):
    ...
    # Save it.
    resp = await fetch("api", "contents", path, method="PUT", body=json.dumps(model))
