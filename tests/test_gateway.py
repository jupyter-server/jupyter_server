...
async def test_gateway_env_options(init_gateway, serverapp):
    ...
    assert (
        serverapp.gateway_config.connect_timeout
        == serverapp.gateway_config.request_timeout
    )
...
async def test_gateway_get_kernelspecs(init_gateway, fetch):
    ...
    with mocked_gateway:
        r = await fetch("api", "kernelspecs", method="GET")
        assert r.code == 200
...
async def test_gateway_get_named_kernelspec(init_gateway, fetch):
    ...
    with mocked_gateway:
        r = await fetch("api", "kernelspecs", "kspec_foo", method="GET")
        assert r.code == 200
        kspec_foo = json.loads(r.body.decode("utf-8"))
        assert kspec_foo.get("name") == "kspec_foo"

        with pytest.raises(tornado.httpclient.HTTPClientError) as e:
            await fetch("api", "kernelspecs", "no_such_spec", method="GET")
...
async def create_session(root_dir, fetch, kernel_name):
    ...
    with mocked_gateway:
        ...
        r = await fetch("api", "sessions", method="POST", body=body)
        assert r.code == 201
...
async def delete_session(fetch, session_id):
    ...
    with mocked_gateway:
        ...
        r = await fetch("api", "sessions", session_id, method="DELETE")
        assert r.code == 204
...
async def is_kernel_running(fetch, kernel_id):
    ...
    with mocked_gateway:
        ...
        r = await fetch("api", "kernels", method="GET")
        assert r.code == 200
...
async def create_kernel(fetch, kernel_name):
    ...
    with mocked_gateway:
        ...
        r = await fetch("api", "kernels", method="POST", body=body)
        assert r.code == 201
        model = json.loads(r.body.decode("utf-8"))
...
async def delete_kernel(fetch, kernel_id):
    ...
    with mocked_gateway:
        # Delete the session (and kernel)
        r = await fetch("api", "kernels", kernel_id, method="DELETE")
        assert r.code == 204
        assert r.reason == "No Content"
