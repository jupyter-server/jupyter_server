...
async def test_from_post(fetch, notebook):
    r = await fetch("api/contents/foo/testnb.ipynb", method="GET",)
    nbmodel = json.loads(r.body.decode())

    r = await fetch("nbconvert", "html", method="POST", body=json.dumps(nbmodel))
    assert r.code == 200
    assert "text/html" in r.headers["Content-Type"]
    assert "Created by test" in r.body.decode()
    assert "print" in r.body.decode()

    r = await fetch("nbconvert", "python", method="POST", body=json.dumps(nbmodel))
    assert r.code == 200
...
async def test_from_post_zip(fetch, notebook):
    ...
    r = await fetch("nbconvert", "latex", method="POST", body=json.dumps(nbmodel))
    assert "application/zip" in r.headers["Content-Type"]
    assert ".zip" in r.headers["Content-Disposition"]
